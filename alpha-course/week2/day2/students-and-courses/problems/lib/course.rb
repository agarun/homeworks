...
class Course
  ...
  # @param self [Course] a course
  # @param course [Course] another course to compare to
  # @return [Boolean] true if two courses conflict
  def conflicts_with?(course)
    self.time_block == course.time_block && (self.days - course.days).size != days.size
  end
end
