...
class Array
  def my_uniq
    unique = {}
    each { |element| unique[element] = true }
    unique.keys
  end
end
...
class Array
  def two_sum
    ...
    each_with_index do |number, idx|
      comp = 0 - number
      comp_idx = complements[comp]
      if comp_idx
        result << [comp_idx, idx]
      else
        complements[number] = idx # i.e. this value can be a complement to another
      end
    end
...
class Array
  ...
  def my_transpose
    # not sure why .dup isn't working here but worked equivalently in #bubble_sort ?!
    self.map(&:clone).my_transpose!
  end
end
...
class Array
  def my_transpose
    rows = size
    cols = first.size
    result = Array.new(cols) { Array.new(rows) } # opposite when transposing
