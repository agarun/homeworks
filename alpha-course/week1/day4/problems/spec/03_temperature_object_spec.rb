...
describe Temperature do
  ...
  describe "utility class methods" do
    it "class method ftoc exists and works properly" do
      expect(Temperature.ftoc(212)).to eq(100)
    end

    it "class method ctof exists and works properly" do
      expect(Temperature.ctof(0)).to eq(32)
    end

    it "ctof is called when converting C to F" do
      expect(Temperature).to receive(:ctof)
      Temperature.new(:c => 100).in_fahrenheit
    end

    it "ftoc is called when converting F to C" do
      expect(Temperature).to receive(:ftoc)
      Temperature.new(:f => 212).in_celsius
    end
  end
