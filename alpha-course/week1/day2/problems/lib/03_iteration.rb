...
# O(n^2) in worst case (n passes), O(n) best case (one pass)
class Array
  def bubble_sort!(&prc)
...
class Array
  def my_map(&prc)
    ...
    my_each { |item| result << prc.call(item) }
  ...
  def my_select(&prc)
    ...
    my_each { |item| result << item if prc.call(item) }
...
def concatenate(strings)
  strings.inject do |accumulator, string|
    accumulator + string
  end
end
